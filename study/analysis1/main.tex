...
\begin{frame}{.}
    ...
    \begin{theorem} \label{th:1.3.10}
        ...
        Therefore, $N(c; \epsilon_j) \subset N(c; \epsilon)$ for all $j \geq j_0$
        Now $x_{k_j}$ is a convergent subsequence of $\{x_k\}$ with $\lim_{j \to \infty} x_{k_j} = c$.
        Thus, for all $j \geq j_0$, the point $x_{k_j}$ is in $N(c; \epsilon)$.
        We conclude that $\{x_{k_j}\}$ converges to $c$ as desired.
...
\begin{frame}{.}
    \begin{theorem} \label{th:1.3.14}
        ...
        \smallskip
        ($\impliedby$) Let the cluster point as $x_0$.
        Assume, for contradiction, that $\{x_k\}$ does not converge.
        Then there exists subsequence $\{x_{k_j}\}$ such that $\forall j, x_{k_j} \notin N(x_0; \epsilon)$.
        Since $\{x_{k_j}\}$ is bounded sequence, by theorem \ref{th:1.3.11}, there exists cluster point $x_0^\prime$.

        Since $\abs{x_{k_j} - x_0} \ge \epsilon$ for all $j$, it follows that $\lim_{j \to \infty} \abs{x_{k_j} - x_0} = \abs{x^\prime_0 - x_0} \geq \epsilon > 0 \implies x_0^\prime \neq x_0$.
        Thus, the sequence $\{x_k\}$ has at least two distinct cluster points ($x_0$ and $x_0^\prime$), which contradicts the hypothesis.
        Therefore, $\{x_k\}$ must converge to $x_0$.
...
\begin{frame}{.}
    \begin{theorem}
        ...
        Proof. Suppose first that $\{x_k\}$ is bounded and divergent. 
    \end{theorem}
\end{frame}
