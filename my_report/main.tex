...
Softmax policy can be one example of score function. 
Assume policy is $\pi_\theta(s,a) \propto e^{\phi(s,a)^\top \theta}$, which is softmax.
Then score function for $\pi_theta(s,a)$ is \cref{eq:eq73}.
\begin{equation} \label{eq:eq73}
    \nabla_\theta \log \pi_\theta(s,a) = \phi(s,a) - \mbb{E}_{\pi_\theta}[\phi(s, \cdot)]
\end{equation}
