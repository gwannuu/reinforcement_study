...
We also consider about off-policy monte carlo estimation, in which behavior policy is different from target policy.
...
\cref{alg:alg7} shows the procedure that policy is evaluated by off-policy manner with ordinal importance sampling.
...
Whereas \cref{alg:alg8} shows the procedure by every-visit manner with weighted important sampling. 
...
\n
$N(s, a) = 0$, for all $s \in \mc{S}$ and for all $a \in \mc{A}(s)$
}\
\While{True}{
...
\begin{equation} \label{eq:eq39}
    \begin{aligned}
        ...
        &\geq \epsilon / m \sum_{a \in \mc{A}}q_\pi(s,a) + (1-\epsilon)\sum_{a \in \mc{A}}\frac{\pi(a|s) - \epsilon / m}{1 - \epsilon}q_\pi(s,a) \quad \left(\sum_{a \in \mc{A}}\frac{\pi(a|s) - \epsilon /m}{1-\epsilon} =1\right)\\
        &= \sum_{a \in \mc{A}}\pi(a|s)q_\pi(s,a) \\
        &= v_\pi(s)
    \end{aligned}
\end{equation}
...
% In this example, we can simply adopt $\epsilon$-greedy of target policy $\pi$ as behavior policy $\mu$. \cref{alg:alg7}.
...
\end{equation}
In this equation, we only need $(S_t,A_t,R_{t+1}, S_{t+1}, A_{t+1})$, so this technique is called \tb{sarsa}.

\begin{algorithm}
    \Input{
        step size $\alpha \in (0,1]$, small $\epsilon > 0$
    }
    \Init{$Q(s,a)$, for all $s \in \mc{S}, a \in \mc{A}(s)$, arbitrarily except that $Q(\text{terminal}, \cdot) = 0$}
...
We can also discuss the \tb{n-step sarsa} algorithm.  By changing \cref{eq:eq43} of algorithm \cref{alg:alg6} as \cref{eq:eq44}, We can simply get n-step sarsa control.
\begin{equation} \label{eq:eq44}
    ...
    \end{gathered}
\end{equation}
In \cref{eq:eq44}, $q^n_t$ is the n-step Q-return.
...
Like off-policy mc control, off-policy TD control needs importance sampling.
...
\label{alg:alg6}
\end{algorithm}
Q-learning is a representative off-policy TD algorithm, and in this subsection, we will examine it. In Q-learning control, target policy $\pi$ is \ti{greedy} and behavior policy $\mu$ is \ti{$\epsilon$-greedy}.
