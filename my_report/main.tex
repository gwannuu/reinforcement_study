...
This is also called as \tb{prediction problem}. If we know the environment's dynamics, then we can see \cref{eq:eq6} as a system of $|\mc{S}|$ linear equations of $|S|$ unknowns. However solving that tremendous linear system involves tedious calculations. For our purpose, iterative solution methods are most suitable. \tb{Iterative update process (Jacobi's method)} for state value function can be expressed by \cref{eq:eq23}.
