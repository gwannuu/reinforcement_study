...
\begin{algorithm}
    ...
    \ForEach{episode, $\{s_1, a_1, r_2, \dots, s_{T-1}, a_{T-1}, r_T\} \sim \pi_\theta$}{
        \For{$t=1,\cdots,T-1$}{
...
\begin{algorithm}
    ...
    \ForEach{episode, $\{s_1, a_1, r_2, \dots, s_{T-1}, a_{T-1}, r_T\}\sim \pi_\theta$}{
        \For{$t=1,\cdots,T-1$}{
            $\delta \gets v_t - V_w(s)$\\
                $w \gets w + \beta \delta \nabla_w V_w(s)$\label{eq:eq77}\\
                $\theta \gets \theta + \alpha \gamma^t \delta$\label{eq:eq78}\\
        }
    }
...
\begin{algorithm} 
    \Input{$\alpha, \beta$, step size}
    \Init{$w$, $\theta$, arbirtary}
    \ForEach{episode}{
        \ForEach{timestep}{
            Initialize $s$, sample action $a \sim \pi(\cdot|s)$,
            sample reward $r$\\
            sample transition $s'$, sample $a'\sim \pi(\cdot|s')$\\
            $\delta \gets r + \gamma Q_w(s',a')-Q_w(s,a)$\\
            $w \gets w + \beta \delta \nabla_w V_w(s)$\\
            $\theta \gets \theta + \alpha \gamma^t Q_w(s,a) \nabla_\theta \log \pi_\theta(s,a)$
        }
    }
